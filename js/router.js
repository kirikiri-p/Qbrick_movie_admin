...
import { addDateInput, addCastInput, renderCharacterCheckboxes, addDirectorInput } from './items.js';
...
function executeGoMovieDetails(mId, isDataUpdate) {
  ...
  if (!isDataUpdate) {
    ...
    const dirContainer = document.getElementById('movie-detail-directors');
    dirContainer.innerHTML = '';
    const dirs = (movie.directors || []).filter(Boolean);
    if (dirs.length > 0) dirs.forEach((n) => addDirectorInput('movie-detail-directors', n));
    else addDirectorInput('movie-detail-directors');
