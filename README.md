...
## ver. 24（重要バグ修正）
- **シーン編集の保存で衣装/小道具が消える不具合を修正**。原因は「誰の」候補(datalist)を `item.id` で要素検索していたが、Excel取込のidに `.`（ドット）が含まれCSSセレクタが壊れ、枠生成が例外で止まり→保存時に空で上書きされていた。datalistの参照を安全な一意ID＋クラス指定に変更。

## ver. 23
- **シーン詳細を閉じたときにスクロール位置を復元**（一覧の先頭に飛ばない）。開いた位置に戻る。
- **Notion連携を複数映画に対応**: 2つの DB に **「映画」列**を追加し、各行に映画タイトルを記入（複数映画を1つのDBで区別・フィルタ可能）。`functions/index.js` 更新後、Notion 側の両DBに「映画」テキスト列を追加して再デプロイが必要。
